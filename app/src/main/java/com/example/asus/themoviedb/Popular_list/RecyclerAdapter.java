...
public class RecyclerAdapter extends RecyclerView.Adapter<RecyclerAdapter.MyViewHolder> {
    ...
    public RecyclerAdapter(ArrayList<PopularMovieList> popularMovieLists , Context context){
    ...
    public void onBindViewHolder(MyViewHolder holder, int position) {
        String releaseDate = popularMovieLists.get(position).getReleaseDate();
        String[] year = releaseDate.split("-");
        String rating = Float.toString(popularMovieLists.get(position).getRating());

        holder.nameTextView.setText(popularMovieLists.get(position).getTitle());
        holder.yearTextView.setText(year[0]);
        holder.ratingTextView.setText(rating);
        Picasso.with(context).load("http://image.tmdb.org/t/p/w500"+popularMovieLists.get(position).getImagePath())
                .into(holder.poster);
